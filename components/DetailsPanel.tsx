  ...
  return (
    <div className="border-b border-gray-200">
      ...
      >
        ...
        <Icon
          name={isOpen ? "chevronsUp" : "chevronsDown"}
...
const isConnector = (entity: CanvasItem | Connector): entity is Connector =>
  "fromId" in entity;
const isTextualItem = (item: CanvasItem): item is TextItem | ShapeItem =>
...
const DetailsPanel: React.FC<DetailsPanelProps> = ({
    ...
    return (
      <div>
        <button
          onClick={() =>
            requiresInput
              ? setShowAiInputFor(isInputVisible ? null : actionKey)
              : handleAiAction(action, false)
          }
        ...
        {isInputVisible && (
          <div className="mt-2 flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") handleAiAction(action, true);
              }}
              className="w-full px-2 py-1 text-sm border border-gray-300 rounded-md"
              placeholder={
                actionKey === "change_tone"
                  ? "예: 전문적으로, 유머러스하게"
                  : "키워드 또는 검색어"
              }
              autoFocus
            />
            <button
              onClick={() => handleAiAction(action, true)}
  ...
  const TabButton: React.FC<{ name: "style" | "ai"; label: string }> = ({
    name,
    label,
  }) => (
    <button
  ...
  // API 관리 탭 및 관련 상태, 핸들러, 크롬 스토리지 연동 완전 삭제

  return (
    <div
      className={`fixed top-0 right-0 h-screen w-80 bg-white shadow-lg z-30 flex flex-col border-l border-gray-200 ${
        className || ""
      }`}
    >
            ...
            <>
              <PanelSection title="변형">
                <StatefulInputField
                  ...
                  onChange={(v) => updateItem({ x: v as number })}
                />
                <StatefulInputField
                  label="Y"
                  id="pos-y"
                  ...
                  onChange={(v) => updateItem({ y: v as number })}
                />
                <StatefulInputField
                  label="너비"
                  id="width"
                  ...
                  min={10}
                  onChange={(v) => updateItem({ width: v as number })}
                />
                <StatefulInputField
                  label="높이"
                  id="height"
                  ...
                  min={10}
                  onChange={(v) => updateItem({ height: v as number })}
                />
              </PanelSection>
              <PanelSection title="모양">
                <div className="grid grid-cols-2 items-center text-sm">
                  <label htmlFor="opacity-slider" className="text-gray-600">
                    투명도
                  </label>
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        updateItem({ opacity: parseFloat(e.target.value) })
                      }
                      className="w-full"
                    />
                ...
                <StatefulInputField
                  ...
                  id="border-radius"
                  type="number"
                  value={entity.borderRadius ?? 0}
                  min={0}
                  onChange={(v) => updateItem({ borderRadius: v as number })}
                  suffix="px"
                />
              </PanelSection>
            </>
          ...
          {textItem && (
            <PanelSection title="배경">
              <ColorPicker
                ...
                id="bg-color"
                color={
                  (textItem.background as SolidBackground).color || "#ffffff"
                }
                onChange={(c) =>
                  updateItem({ background: { type: "solid", color: c } })
                }
              />
            </PanelSection>
          )}
          {shapeItem && (
            <PanelSection title="테두리">
              <ColorPicker
                label="색상"
                id="border-color"
                color={shapeItem.border.color}
                onChange={(c) =>
                  updateItem({ border: { ...shapeItem.border, color: c } })
                }
              />
              <StatefulInputField
                label="두께"
                id="border-width"
                type="number"
                value={shapeItem.border.width}
                min={0}
                onChange={(v) =>
                  updateItem({
                    border: { ...shapeItem.border, width: v as number },
                  })
                }
                suffix="px"
              />
              <div className="grid grid-cols-2 items-center text-sm">
                <label htmlFor="border-style" className="text-gray-600">
                  스타일
                </label>
                <select
                  id="border-style"
                  value={shapeItem.border.style}
                  onChange={(e) =>
                    updateItem({
                      border: {
                        ...shapeItem.border,
                        style: e.target.value as BorderStyleType,
                      },
                    })
                  }
                  className="w-full px-2 py-1 bg-gray-100 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
          ...
          {connector && (
            <PanelSection title="연결선 스타일">
              <StatefulInputField
                ...
                onChange={(v) => updateConnector({ label: v as string })}
              />
              <ColorPicker
                label="라벨 색상"
                id="conn-label-color"
                color={connector.labelColor || "#000000"}
                onChange={(c) => updateConnector({ labelColor: c })}
              />
              <StatefulInputField
                label="라벨 크기"
                id="conn-label-size"
                type="number"
                value={connector.labelFontSize || 14}
                min={8}
                onChange={(v) =>
                  updateConnector({ labelFontSize: v as number })
                }
                suffix="px"
              />
              ...
              <ColorPicker
                label="선 색상"
                id="conn-color"
                color={connector.color || "#4b5563"}
                onChange={(c) => updateConnector({ color: c })}
              />
              <StatefulInputField
                label="선 두께"
                id="conn-width"
                type="number"
                value={connector.strokeWidth || 3}
                min={1}
                onChange={(v) => updateConnector({ strokeWidth: v as number })}
                suffix="px"
              />
              <div className="grid grid-cols-2 items-center text-sm">
                <label htmlFor="conn-style" className="text-gray-600">
                  선 스타일
                </label>
                <select
                  id="conn-style"
                  ...
                  onChange={(e) =>
                    updateConnector({
                      style: e.target.value as ConnectorStyleType,
                    })
                  }
                  className="w-full px-2 py-1 bg-gray-100 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
          ...
          <PanelSection title="AI 텍스트 편집">
            {isGeneratingAIContentForThisItem ? (
              ...
              <div className="space-y-2">
                ...
                />
                <AiActionButton
                  action={onChangeTextTone}
                  label="다른 문체로 변경..."
                  requiresInput
                  actionKey="change_tone"
                />
                <AiActionButton
                  action={onAddKeywordParagraph}
                  label="키워드 문단 추가..."
                  requiresInput
                  actionKey="add_keyword"
                />
                <AiActionButton
                  action={onInformationSearch}
                  label="정보 검색 및 통합..."
                  requiresInput
                  actionKey="search_info"
                />
              </div>
            )}
          </PanelSection>
