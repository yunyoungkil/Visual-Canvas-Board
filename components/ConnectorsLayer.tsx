...
interface ConnectorsLayerProps {
  ...
  items: CanvasItem[];
  getHandlePosition: (
    item: CanvasItem | { x: number; y: number; width: number; height: number },
...
const ConnectorsLayer: React.FC<ConnectorsLayerProps> = React.memo(
  ({
    ...
    return (
      <svg
        className="absolute"
        viewBox={`-${C.CANVAS_WORLD_SIZE / 2} -${C.CANVAS_WORLD_SIZE / 2} ${
          C.CANVAS_WORLD_SIZE
        } ${C.CANVAS_WORLD_SIZE}`}
        style={{
          overflow: "visible",
      ...
      >
        {connectors.map((conn) => {
          const fromItem =
            itemMap.get(conn.fromId) || groupBoundsMap.get(conn.fromId);
          const toItem =
            itemMap.get(conn.toId) || groupBoundsMap.get(conn.toId);
          if (!fromItem || !toItem) return null;

          let bestPath = {
            fromHandle: allHandles[0],
            toHandle: allHandles[0],
            dist: Infinity,
          };
          ...
          const derivative = {
            x:
              3 * (1 - t) ** 2 * (cp1.x - fromPos.x) +
              6 * (1 - t) * t * (cp2.x - cp1.x) +
              3 * t ** 2 * (toPos.x - cp2.x),
            y:
              3 * (1 - t) ** 2 * (cp1.y - fromPos.y) +
              6 * (1 - t) * t * (cp2.y - cp1.y) +
              3 * t ** 2 * (toPos.y - cp2.y),
          };
          ...
          return (
            <g
              key={conn.id}
              data-id={conn.id}
              className="connector-group pointer-events-auto cursor-pointer"
              onMouseEnter={() => onConnectorMouseEnter(conn.id)}
              onMouseLeave={onConnectorMouseLeave}
              onClick={(e) => onConnectorClick(e, conn.id)}
              onDoubleClick={() =>
                onConnectorDoubleClick(conn, midpoint, angle)
              }
            >
              <path
                d={pathData}
              ...
              {conn.label && !isEditing && (
                <text
                  ...
                  y={midpoint.y}
                  dy={-5}
                  transform={`rotate(${effectiveAngle},${midpoint.x},${midpoint.y})`}
                  textAnchor="middle"
                  fill={conn.labelColor || "#000000"}
                  style={{
                    fontSize: `${labelFontSize}px`,
                    paintOrder: "stroke",
                    stroke: isHovered || isSelected ? "#e5e7eb" : "white",
                    strokeWidth: `4px`,
