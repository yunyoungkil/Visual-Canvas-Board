...
interface CanvasItemComponentProps {
  ...
  onConnectionStart: (e: React.MouseEvent, fromId: string, fromHandle: HandlePosition, fromPos: Point) => void;
  onContentUpdate: (id: string, content: string, shouldCommit?: boolean, finalHeight?: number) => void;
  onUpdateItem: (itemId: string, updates: Partial<CanvasItem>, shouldCommit?: boolean) => void;
...
const getGradientCss = (background: GradientBackground): string => {
    const sortedStops = [...(background.stops || [])].sort((a, b) => a.position - b.position);
    const colorStopsString = sortedStops.map(stop => `${stop.color} ${stop.position * 100}%`).join(', ');
...
// The TextStyle extension is already included in StarterKit. Including it again causes a conflict.
    ...
    onUpdate: ({ editor }) => {
        ...
        const newAlign = ['left', 'center', 'right', 'justify'].find(align => editor.isActive({ textAlign: align })) as 'left' | 'center' | 'right' | 'justify' | undefined;
        
        if (newAlign && (item as TextItem | ShapeItem).textAlign !== newAlign) {
  ...
  }, [editor, isTextualItem ? (item as TextItem | ShapeItem).textAlign : null]);
  ...
  useEffect(() => {
    ...
    const canShowGenerateDraftButton = isSelected && currentTextValue.trim().length > 0 && currentTextValue.trim().length < 200 && !htmlContent.includes('<br>');
    const canShowUpdateDraftButton = isSelected && !isEditing && currentTextValue.trim().length >= 200 && hasConnections;

    if (itemContentRef.current && isSingleSelection && (canShowGenerateDraftButton || canShowUpdateDraftButton)) {
        onShowItemAiToolbar(item, itemContentRef.current.getBoundingClientRect());
    } else {
        onHideItemAiToolbar();
    }
  }, [item, isSelected, isSingleSelection, isEditing, hasConnections, onShowItemAiToolbar, onHideItemAiToolbar, isTextualItem]);
  ...
  const getShapePath = (shape: ShapeItem) => {
    ...
    switch (shape.shape) {
        case 'ellipse':
            return `M ${b},${height / 2} C ${b},${b} ${width-b},${b} ${width-b},${height / 2} C ${width-b},${height-b} ${b},${height-b} ${b},${height/2} Z`;
        case 'diamond':
            return `M ${width/2},${b} L ${width-b},${height/2} L ${width/2},${height-b} L ${b},${height/2} Z`;
        case 'rectangle':
        default:
            return `M ${b},${b} L ${width-b},${b} L ${width-b},${height-b} L ${b},${height-b} Z`;
    }
  };

  const showConnectionHandles = (isSelected && isSingleSelection) || isHoveredForConnection;
  ...
  if (item.type === 'text' || item.type === 'shape') {
    if (item.background.type === 'solid') {
      ...
      itemBackgroundStyle.backgroundImage = getGradientCss(item.background as GradientBackground);
    }
  }
  ...
  return (
    <div
      ...
      onMouseUp={e => { e.stopPropagation(); onMouseUp(e, item.id); }}
      onDoubleClick={() => itemContentRef.current && onDoubleClick(item, itemContentRef.current.getBoundingClientRect())}
    >
      {isGeneratingAIContentForThisItem && (
      ...
      <div
        className={`w-full h-full transition-shadow duration-100 ${isSelected ? 'ring-2 ring-blue-500' : 'shadow-md hover:shadow-lg'} ${isEditing ? 'ring-2 ring-blue-500 shadow-lg' : ''} ${item.type === 'image' ? 'bg-white' : ''}`}
        style={{
        ...
        ref={itemContentRef}
      >
        {item.type === 'image' && <img src={item.src} alt="" className="w-full h-full object-cover pointer-events-none" draggable={false}/>}
        
        {item.type === 'shape' && (
          <>
            <div className="w-full h-full relative pointer-events-none">
              <svg width="100%" height="100%" viewBox={`0 0 ${item.width} ${item.height}`} style={{overflow: 'visible', position: 'absolute', top: 0, left: 0}}>
                <path
                  d={getShapePath(item)}
                  fill="transparent"
                  stroke={item.border.color}
                  strokeWidth={item.border.width}
                  strokeDasharray={item.border.style === 'dashed' ? '8, 8' : item.border.style === 'dotted' ? '2, 6' : 'none'}
                />
              </svg>
               {/* Removed justifyContent. Text alignment is now fully controlled by Tiptap. `flex` and `items-center` are for vertical centering. */}
               <div onMouseDown={handleEditorContainerMouseDown} className="absolute inset-0 flex items-center p-2 break-words overflow-y-auto" style={{pointerEvents: 'auto'}}>
        ...
        {item.type === 'text' && (
           <div onMouseDown={handleEditorContainerMouseDown} className="w-full h-full p-2 break-words overflow-y-auto">
              {editor && <EditorContent editor={editor} />}
          </div>
        )}
      </div>
      ...
      {isSelected && isSingleSelection && !isEditing && (
        <div
            ...
            style={{ width: `${16/scale}px`, height: `${16/scale}px`, right: `${-8/scale}px`, bottom: `${-8/scale}px`}}
          ...
          {(['top', 'bottom', 'left', 'right'] as const).map(pos => {
              ...
              return (
                  <div
                      ...
                      onMouseDown={(e) => onConnectionStart(e, item.id, pos, handlePos)}
                  >
                      <div
