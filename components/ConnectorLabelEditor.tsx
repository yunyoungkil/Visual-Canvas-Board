...
interface ConnectorLabelEditorProps {
    ...
    angle: number;
    onEndEdit: (id: string, updates: { text: string, color: string, fontSize: number }) => void;
    onCancel: () => void;
...
const ConnectorLabelEditor: React.FC<ConnectorLabelEditorProps> = ({ 
    ...
    return (
        <div
            ref={panelRef}
            className="fixed p-2 space-y-2 bg-white border border-blue-500 rounded-md shadow-lg" // Changed to fixed
            style={{
                left: position.x,
                top: position.y,
                transform: `translate(-50%, -50%) rotate(${angle}deg) translateY(-30px)`, // Still need rotation and offset
                ...style
            }}
