...
const CanvasEditor: React.FC = () => {
  ...
  // localStorage에서 저장된 캔버스 로드
  useEffect(() => {
    const loadCanvasData = localStorage.getItem("loadCanvas");
    if (loadCanvasData) {
      try {
        const { items: loadedItems, connectors: loadedConnectors } = JSON.parse(loadCanvasData);
        canvasStateAndActions.setItems(loadedItems);
        canvasStateAndActions.setConnectors(loadedConnectors);
        localStorage.removeItem("loadCanvas");
      } catch (error) {
        console.error("캔버스 로드 실패:", error);
      }
    }
  }, []);
  ...
  // 카드 리스트 패널 토글
  const handleToggleCardListPanel = useCallback((category: CardCategory | null) => {
    setActiveCardCategory(category);
  }, []);

  // 카드에서 캔버스 로드
  const handleLoadCanvasFromCard = useCallback(
    (loadedItems: CanvasItem[], loadedConnectors: Connector[]) => {
      canvasStateAndActions.setItems(loadedItems);
      canvasStateAndActions.setConnectors(loadedConnectors);
      canvasStateAndActions.commitState(loadedItems, loadedConnectors);
  ...
  // 저장 다이얼로그 열기
  const handleOpenSaveDialog = useCallback((category: CardCategory) => {
    setSaveDialogData({
      title: "",
      category,
      description: "",
  ...
  const handleSaveCanvas = useCallback(async () => {
    ...
    try {
      ...
      if (!exportRef.current || !ctx) return;
      ...
      // HTML을 이미지로 변환
      const data = new XMLSerializer().serializeToString(exportRef.current);
      const img = new Image();
      const svg = `
      ...
      img.onload = async () => {
        ...
        await firebaseStorage.saveCanvas(
          saveDialogData.title,
          saveDialogData.category,
          items,
          connectors,
  ...
  }, [saveDialogData, items, connectors, firebaseStorage, exportRef]);
  ...
  return (
    ...
    >
      ...
      <Header
        ...
        className="z-50"
        onSaveCanvas={handleOpenSaveDialog}
      />
      ...
      <LeftToolbar
        ...
        className="z-50"
        onToggleCardListPanel={handleToggleCardListPanel}
      />
      ...
      {activeCardCategory && (
        <CardListPanel
          category={activeCardCategory}
          canvases={firebaseStorage.getCanvasesByCategory(activeCardCategory)}
          onClose={() => setActiveCardCategory(null)}
          onLoadCanvas={handleLoadCanvasFromCard}
          onDeleteCanvas={firebaseStorage.deleteCanvas}
        />
      )}
      ...
      {isSaveDialogOpen && (
        <div className="fixed inset-0 bg-black/50 z-[1000] flex items-center justify-center">
          <div className="bg-white rounded-xl p-6 w-[500px] max-w-[90vw] shadow-2xl">
            <h2 className="text-xl font-bold mb-4">캔버스 저장</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSaveDialogData({ ...saveDialogData, title: e.target.value })}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {(['scrap', 'idea', 'planning'] as CardCategory[]).map((cat) => (
                    <button
                      key={cat}
                      onClick={() => setSaveDialogData({ ...saveDialogData, category: cat })}
                      className={`flex-1 px-4 py-2 rounded-lg transition-colors ${
                        saveDialogData.category === cat
                          ? 'bg-blue-500 text-white'
                          : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                      }`}
                    >
                      {cat === 'scrap' ? '스크랩' : cat === 'idea' ? '아이디어' : '기획/작성 중'}
                    </button>
                  ))}
                </div>
              </div>
