...
export const useCanvasInteraction = ({
  ...
  const [draggingState, setDraggingState] = useState<{
    id: string;
    dragStartPos: Point;
    itemStartPositions: Map<string, Point>;
  } | null>(null);
  ...
  const [editingConnectorState, setEditingConnectorState] = useState<{
    id: string;
    text: string;
    pos: Point;
    angle: number;
  ...
  const [hoveredItemIdForConnection, setHoveredItemIdForConnection] = useState<
    string | null
  >(null);
  ...
  const snapToGrid = useCallback(
    (value: number): number =>
      isGridSnapActive ? Math.round(value / C.GRID_SIZE) * C.GRID_SIZE : value,
    [isGridSnapActive]
  );
  ...
  const zoomToPoint = useCallback(
    (newScale: number, point: Point) => {
      const mouseCanvasPosBefore = screenToCanvas(point);
      setViewOffset({
        x: point.x - mouseCanvasPosBefore.x * newScale,
        y: point.y - mouseCanvasPosBefore.y * newScale,
      });
      setScale(newScale);
    },
  ...
  const handleZoomIn = useCallback(
    () =>
      canvasRef.current &&
      zoomToPoint(Math.min(C.MAX_ZOOM, scale + 0.1), {
  ...
  const handleZoomOut = useCallback(
    () =>
      canvasRef.current &&
      zoomToPoint(Math.max(C.MIN_ZOOM, scale - 0.1), {
        x: canvasRef.current.clientWidth / 2,
        y: canvasRef.current.clientHeight / 2,
      }),
    [scale, zoomToPoint]
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      const newScale = Math.max(
        C.MIN_ZOOM,
        Math.min(C.MAX_ZOOM, scale * (1 - e.deltaY * 0.002))
      );
  ...
  const handleItemMouseDown = useCallback(
    (e: React.MouseEvent<HTMLDivElement>, id: string) => {
      ...
      const isUnitSelected = interactionUnitIds.every((uid) =>
      ...
      if (e.shiftKey) {
        if (isUnitSelected) {
          ...
          newSelectionIds = [
            ...new Set([...selectedItemIds, ...interactionUnitIds]),
          ];
        }
      } else {
        if (!isUnitSelected) {
      ...
      const shouldDragSelection = primaryDragUnit.every((uid) =>
        newSelectionIds.includes(uid)
      );
      const itemsToConsiderForDrag = shouldDragSelection
        ? newSelectionIds
        : primaryDragUnit;

      itemsToConsiderForDrag.forEach((dragId) => {
        const item = items.find((i) => i.id === dragId);
        // Only expand to full group if Ctrl/Cmd is pressed
        if ((e.ctrlKey || e.metaKey) && item?.groupId) {
          items
            .filter((i) => i.groupId === item.groupId)
            .forEach((groupItem) => dragSet.add(groupItem.id));
        } else if (item) {
          dragSet.add(item.id);
        }
      });
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (panningState) {
        ...
        setViewOffset({
          x: panningState.startViewOffset.x + dx,
          y: panningState.startViewOffset.y + dy,
        });
        ...
        const movingItems = items.filter((i) =>
          draggingState.itemStartPositions.has(i.id)
        );
        const staticItems = items.filter(
          (i) => !draggingState.itemStartPositions.has(i.id)
        );
        ...
        if (closestHandle) {
          ...
          setConnectingState({
            ...connectingState,
            currentPos: closestHandle.pos,
          });
        } else {
          setHoveredItemIdForConnection(null);
          setConnectingState({
            ...connectingState,
            currentPos: currentCanvasPos,
          });
        }
      }
    },
  ...
  const handleConnectorLabelEdit = useCallback(
    (conn: Connector, midpoint: Point, angle: number) => {
      setEditingConnectorState({
        id: conn.id,
        text: conn.label || "",
        pos: canvasToScreen(midpoint),
        angle,
        color: conn.labelColor || "#000000",
        fontSize: conn.labelFontSize || 14,
      });
    },
    [canvasToScreen]
  );
  ...
  const handleMouseUp = useCallback(
    (e: React.MouseEvent | MouseEvent, itemId?: string) => {
      setSnapLines([]);
      ...
      const isSimpleClick =
        ...
        Math.hypot(
          endPos.x - clickStartPos.current.x,
          endPos.y - clickStartPos.current.y
        ) < 5;
  ...
  const handleConnectorLabelUpdate = useCallback(
    (
      id: string,
  ...
  const handleDelete = useCallback(() => {
    ...
    const newConnectors = connectors.filter(
      (conn) =>
        !idsToDelete.has(conn.fromId) &&
        !idsToDelete.has(conn.toId) &&
        conn.id !== selectedConnectorId
    );
  ...
  const handlePaste = useCallback(() => {
    ...
    const pastePos = contextMenuState
      ? screenToCanvas({ x: contextMenuState.x, y: contextMenuState.y })
      : { x: 0, y: 0 };
    const newItems: CanvasItem[] = clipboardRef.current.map(
      (itemToPaste, index) => {
        return {
          ...itemToPaste,
          id: crypto.randomUUID(),
  ...
  useEffect(() => {
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      if (e.key === " ") setIsSpacePanning(false);
    };
  ...
  return {
    ...
    onConnectionStart: (
      ...
      e.stopPropagation();
      window.getSelection()?.empty();
      setConnectingState({ fromId, fromHandle, fromPos, currentPos: fromPos });
    },
    handleContentUpdate,
    handleCanvasMouseDown,
    handleConnectorLabelEdit,
    handleGroup,
    handleUngroup,
    handleDelete,
    handleCopy,
